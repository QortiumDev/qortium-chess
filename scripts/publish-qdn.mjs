...
const ENV_PREFIX = 'QORTIUM_CHESS';
const DEFAULT_NODE_API_URL = 'http://127.0.0.1:24891';
const DEFAULT_NAME = 'Chess';
const DEFAULT_IDENTIFIER = 'Chess';
const DEFAULT_TITLE = 'Chess';
const DEFAULT_DESCRIPTION = 'A QDN app for Qortium: Chess.';
...
// These default to the RUNNING node's key, not a checked-out copy — a stale key
// fails as an opaque 401 at publish time. Paths match qortium-chat's script;
// the old `~/git/qortium/preview/...` defaults predate the workspace reorg and
...
async function ensureNameRegistered(name, account) {
  ...
  const rawRegisterBytes58 = buildRegisterNameRawBytes58({
    ...
    data: JSON.stringify({
      app: DEFAULT_TITLE,
      purpose: 'QDN app preview for Chess',
    }),
  });
